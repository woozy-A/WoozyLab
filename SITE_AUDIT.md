...
- 대상: KeyPic, GlassLingo, 오랜나무. 기존 15개 공개 주소 유지 + 오랜나무 3개 주소 추가.
- 원인: 공통 CSS를 도입해도 HTML 메뉴/푸터는 15번 중복되었고 GlassLingo 소개에 별도 전역 CSS가 남아 있었음.
- 수정: 공통 생성 틀, 앱별 색 분리, 본문/앱 정보 분리, 생성 결과 일치 검사. 모든 작업의 시작점을 `AGENTS.md`에 명시.
- 공통 개인정보 한 장으로 합치지 않고 앱별 정책을 유지. 기존 두 앱의 정책 의미와 날짜는 변경하지 않음.
- 오랜나무 승인 지원·정책 문서/아이콘/실제 Release 시뮬레이터 화면을 명시적으로 가져옴. AR 출시 기능 홍보 없음.
- 고유한 렌즈를 흉내 낸 GlassLingo 장식 화면은 제거하고 실제 앱 아이콘을 사용. 제품 기능/권한/조건부 개인정보 문구는 보존.
- 향후 앱 추가는 `apps` + 승인 문서 + 실제 assets로 수행. 기존 메뉴를 복사해 페이지별 변형하지 않음.
- 현재 남은 확인: App Store 공개 링크, KeyPic 정확한 권한 목록, GlassLingo 출시 전 실제 동작과 정책 재대조, 오랜나무 앱 URL 전환.
- 배포/검증 결과는 `CURRENT_STATUS.md`에서 확인. 아래는 2026-07-18 당시 기록이며 최신 검증 결과가 아님.
